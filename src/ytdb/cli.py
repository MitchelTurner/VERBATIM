...
def _register_commands() -> None:
    if getattr(cli, "_ytdb_commands_registered", False):
        return
    ...
    def list_channels() -> None:
        ...
        for channel in channels:
            with repository.session() as session:
                transcript_count = repository.count_transcripts_for_channel(
                    session, channel.id
                )
            click.echo(
    ...
    def sync(
        account: str, max_videos: int | None, languages: tuple[str, ...], force: bool
    ) -> None:
    ...
    cli._ytdb_commands_registered = True
